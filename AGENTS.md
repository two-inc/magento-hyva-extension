...
survive": its re-render never touches this form. The same hook puts the caret back: a morph
patches the company input in place and Chrome blurs the replaced element to
nothing, so the sweep re-focuses the company field — only where the drop left
focus unplaced, and only for the control that was holding it (ABN-554).
